...
# ---------- 1) kill old schedule completely ----------
node << 'NODEEOF'
const fs = require('fs')
const p = 'src/lib/schedule.ts'
let s = fs.readFileSync(p, 'utf8')
s = s.replace(/export async function sendDueTelegram[\s\S]*?^}/m, 'export async function sendDueTelegram() { return [] }')
s = s.replace(/export async function sendDueInstagram[\s\S]*?^}/m, 'export async function sendDueInstagram() { return [] }')
fs.writeFileSync(p, s)
console.log('✅ old schedule disabled')
NODEEOF

# ---------- 2) new hourly cron: send 1 random prompt to TG + IG ----------
mkdir -p src/app/api/cron/hourly
cat > src/app/api/cron/hourly/route.ts << 'EOF'
...
EOF
echo "✅ hourly cron created"

# ---------- 3) remove old daily5 references ----------
node << 'NODEEOF'
const fs = require('fs')
const p = 'src/app/api/cron/article/route.ts'
let s = fs.readFileSync(p, 'utf8')
s = s.replace("import { buildDaily5 } from '@/lib/daily5'", '')
s = s.replace('const schedule = await buildDaily5(searchParams.get(\'test\') === \'1\').catch(() => null)', 'const schedule = { built: false }')
fs.writeFileSync(p, s)
console.log('✅ article cron: daily5 removed')
NODEEOF
...
let s = fs.readFileSync(p, 'utf8')
s = s.replace("import { sendDueDaily5 } from '@/lib/daily5'", '')
s = s.replace('await sendDueDaily5().catch(() => {})', '')
fs.writeFileSync(p, s)
console.log('✅ telegram cron: daily5 removed')
NODEEOF
