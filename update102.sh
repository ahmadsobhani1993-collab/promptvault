...
# ---------- 1) qwen: single-model function + chain ----------
...
EOF
echo "✅ qwen: qwenSingle exported"

# ---------- 2) article route: two-step chained ----------
...
export async function GET(req: Request) {
  ...
  // ---------------- STEP 2: image + publish ----------------
  const draftRaw = await getSetting('article_draft', '')
  if (!draftRaw) return NextResponse.json({ ok: false, error: 'no draft — run step=1 first' }, { status: 400 })
  let raw = ''
...
EOF
echo "✅ article: two-step chained (no more timeout)"
