...
# ---------- 1) Admin prompt preview page ----------
mkdir -p src/app/admin/prompts/[id]/preview

cat > src/app/admin/prompts/[id]/preview/page.tsx << 'EOF'
...
EOF
echo "✅ Admin prompt preview page created"

# ---------- 2) Update admin prompts page to link to preview ----------
node << 'NODEEOF'
const fs = require('fs')
const p = 'src/app/admin/prompts/page.tsx'
let s = fs.readFileSync(p, 'utf8')

// Change the link from /prompts/[slug] to /admin/prompts/[id]/preview
s = s.replace(
  /href=\{'\/prompts\/' \+ p\.slug\}/,
  "href={'/admin/prompts/' + p.id + '/preview'}"
)

fs.writeFileSync(p, s)
console.log('✅ Admin prompts: links to preview page')
NODEEOF
