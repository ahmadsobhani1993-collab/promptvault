...
import {
  ...
  type Seg, type Style, type Fx, type TextDirection, type TextAlign,
  mkWords, loadFont, getAnimationState, resolveDirection, resolveAlign,
...
export default function SubtitleStudio({ videoUrl, segments, setSegments }: Props) {
  ...
  const onSubPointerDown = (e: PointerEvent) => {
    e.preventDefault()
  ...
  const animation = current
    ? getAnimationState(
        current.fx,
        Math.max(0, time - current.start),
        current.end - current.start,
        Math.max(1, videoFrame.width)
      )
    : null

  const resolvedDirection = current
    ? resolveDirection(style.direction, current.text)
    : 'rtl'

  const resolvedAlign = resolveAlign(style.align, resolvedDirection)
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        ...
        <div className="space-y-3 lg:col-span-3">
                ...
                >
                  <span
                    ...
                    style={{
                      ...
                      transform: animation
                        ? `translate(${animation.translateX}px, ${animation.translateY}px) scale(${animation.scale})`
                        : undefined,
          ...
          {showAdv && (
            <div className="grid grid-cols-2 gap-3 rounded-xl border border-white/10 bg-zinc-900/60 p-3 text-xs sm:grid-cols-4 lg:grid-cols-5">
              <div>
              ...
              <div>
                <div className="mb-1 text-white/40">جهت متن</div>
                <select
                  value={style.direction}
                  onChange={(e) => setStyle({ ...style, direction: e.target.value as TextDirection })}
                  className="w-full rounded-lg border border-white/10 bg-black/40 p-1.5 text-white/80"
                >
              ...
              <div>
                <div className="mb-1 text-white/40">تراز متن</div>
                <select
                  value={style.align}
                  onChange={(e) => setStyle({ ...style, align: e.target.value as TextAlign })}
                  className="w-full rounded-lg border border-white/10 bg-black/40 p-1.5 text-white/80"
                >
              ...
              <div>
                <div className="mb-1 text-white/40">موقعیت X: {style.x == null ? 'پیش‌فرض' : `${style.x.toFixed(1)}٪`}</div>
                <input
                  type="range" min={2} max={98} step={0.5}
                  value={style.x ?? 50}
                  onChange={(e) => setStyle({ ...style, x: Number(e.target.value) })}
                  className="w-full accent-amber-500"
                />
              </div>

              <div>
                <div className="mb-1 text-white/40">موقعیت Y: {style.y == null ? 'پیش‌فرض' : `${style.y.toFixed(1)}٪`}</div>
                <input
                  type="range" min={2} max={98} step={0.5}
                  value={style.y ?? 90}
                  onChange={(e) => setStyle({ ...style, y: Number(e.target.value) })}
                  className="w-full accent-amber-500"
                />
              </div>
            </div>
