...
# ---------- 1) Fix channel ID to use @promptsfa1 ----------
mkdir -p src/app/api/debug/set-channel
cat > src/app/api/debug/set-channel/route.ts << 'EOF'
...
EOF
echo "✅ Set channel route created"

# ---------- 2) Update upload route to use saved channel ----------
node << 'NODEEOF'
const fs = require('fs')
const p = 'src/app/api/upload-to-telegram/route.ts'
let s = fs.readFileSync(p, 'utf8')

// Replace channel username logic to use 'promptsfa1' directly
s = s.replace(
...
console.log('✅ Upload route: hardcoded to @promptsfa1')
NODEEOF

echo ""
echo "===== NEXT STEPS ====="
echo "1. Run this to save correct channel:"
echo "   https://promptsfa.ir/api/debug/set-channel?key=pv-cron-8x2m1q"
echo ""
echo "2. Test upload to @promptsfa1"
echo "=========================="
