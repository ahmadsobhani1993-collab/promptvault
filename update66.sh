...
# ---------- 1) animated hero canvas ----------
cat > src/components/hero-canvas.tsx << 'EOF'
'use client'
...
# ---------- 2) page.tsx: canvas + image-only hot prompts ----------
node << 'NODEEOF'
const fs = require('fs')
const p = 'src/app/page.tsx'
let s = fs.readFileSync(p, 'utf8')

if (!s.includes('HeroCanvas')) {
  s = s.replace(
  ...
  console.log('✅ page: HeroCanvas added')
}

if (!s.includes('imgPrompts')) {
  s = s.replace(
    'const trending: typeof prompts = []',
    "const imgPrompts = prompts.filter((p) => (p as any).type === 'IMAGE')\n  const trending: typeof prompts = []"
  )
  s = s.replaceAll('for (const p of prompts) {', 'for (const p of imgPrompts) {')
  console.log('✅ page: hot prompts = IMAGE only')
}

fs.writeFileSync(p, s)
NODEEOF

# ---------- 3) header: icons + no plus ----------
node << 'NODEEOF'
const fs = require('fs')
const p = 'src/components/layout/header.tsx'
let s = fs.readFileSync(p, 'utf8')

if (!s.includes('CategoryIcon')) {
  s = s.replace(
  ...
  console.log('✅ header: CategoryIcon in dropdown')
}

if (s.includes('> + {L(locale, \'ارسال\', \'Submit\')}')) {
  s = s.replace("> + {L(locale, 'ارسال', 'Submit')}", "> {L(locale, 'ارسال', 'Submit')}")
  console.log('✅ header: plus removed')
}
